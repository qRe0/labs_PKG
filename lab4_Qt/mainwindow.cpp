...
void MainWindow::showBrCircle(){
    ...
    while(true){
        ...
        if(qAbs(p0.x()) + qAbs(radius) >= this->height()/24 || qAbs(p0.y()) + qAbs(radius) >= this->width()/40){
            outOfRangeError();
        }else{
            break;
        }
...
void MainWindow::showBrSegment(){
    ...
    while(true){
        ...
        if(qAbs(p0.x()) >= this->height()/24 || qAbs(p0.y()) >= this->width()/40 || qAbs(p1.x()) >= this->height()/24 || qAbs(p1.y()) >= this->width()/40){
            outOfRangeError();
        }else{
            break;
        }
